...
function moveStoreSelector() {
  const storeSelector = document.querySelector(
    ".dynamic-form-field.dynamic-form-field--field_52"
  );
  ...
  if (storeSelector) {
    ...
    // Cambiar titulo
    const optionalText = storeSelector.querySelector(
    ...
    const ulShippingOptions = document.querySelector(
      "#checkout-shipping-options ul.form-checklist"
    );
    if (ulShippingOptions) {
      ...
      const checkShippingOption = () =>
        setStoreSelector(storeSelector, !sendByWeightOption?.checked);
